...
QBCore.Functions.CreateCallback('A7md-scoreboard:server:GetActivity', function(source, cb)
    local PoliceCount = 0
...
QBCore.Functions.CreateCallback('A7md-scoreboard:server:GetConfig', function(source, cb)
    cb(Config.IllegalActions)
end)

QBCore.Functions.CreateCallback('A7md-scoreboard:server:GetPlayersArrays', function(source, cb)
    local players = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then 
...
RegisterServerEvent('A7md-scoreboard:server:SetActivityBusy')
AddEventHandler('A7md-scoreboard:server:SetActivityBusy', function(activity, bool)
    Config.IllegalActions[activity].busy = bool
    TriggerClientEvent('A7md-scoreboard:client:SetActivityBusy', -1, activity, bool)
end)
